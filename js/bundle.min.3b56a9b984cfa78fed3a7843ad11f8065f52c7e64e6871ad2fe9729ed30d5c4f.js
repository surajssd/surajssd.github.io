...
function search(query){return idx.search(query);};(function(){let form=document.querySelector('.new-comment');if(form){form.addEventListener('submit',function(){form.classList.add('loading');form.querySelector('input[type="submit"]:enabled').classList.add('hidden');form.querySelector('input[type="submit"]:disabled').classList.remove('hidden');let api='';let gitProvider='';let username='';let repo='';let branch='';let url=['https:/',api,'v3/entry',gitProvider,username,repo,branch,'comments'].join('/');let formObj=Object.fromEntries(new FormData(form));let xhrObj={fields:{},options:{}};Object.entries(formObj).forEach(([key,value])=>{let a=key.indexOf('['),b=key.indexOf('reCaptcha');if(a==-1){xhrObj[key]=value;}else if(a==6||(a==7&&b==-1)){xhrObj[key.slice(0,a)][key.slice(a+1,-1)]=value;}else{xhrObj.options.reCaptcha=xhrObj.options.reCaptcha||{};xhrObj.options.reCaptcha[key.slice(b+11,-1)]=value;}});let formData=JSON.stringify(xhrObj);let xhr=new XMLHttpRequest();xhr.open('POST',url);xhr.setRequestHeader('Content-Type','application/json;charset=UTF-8');xhr.setRequestHeader('X-Requested-With','XMLHttpRequest');xhr.onreadystatechange=function(){if(xhr.readyState===XMLHttpRequest.DONE){let status=xhr.status;if(status>=200&&status<400){formSubmitted();}else{formError(err);}}};xhr.send(formData);return false;});function formSubmitted(){showAlert('success');setTimeout(function(){clearForm()},3000);form.classList.remove('loading');}
function formError(err){console.log(err);showAlert('failed');form.classList.remove('loading');}
function showAlert(msg){if(msg=='success'){form.querySelector('.submit-notice').classList.add('submit-success')
form.querySelector('.submit-success').classList.remove('hidden');form.querySelector('.submit-failed').classList.add('hidden');}else{form.querySelector('.submit-notice').classList.add('submit-failed')
form.querySelector('.submit-success').classList.add('hidden');form.querySelector('.submit-failed').classList.remove('hidden');}
form.querySelector('input[type="submit"]:enabled').classList.remove('hidden');form.querySelector('input[type="submit"]:disabled').classList.add('hidden');}
function clearForm(){resetReplyTarget();form.querySelector('.submit-notice').classList.remove('.submit-success');form.querySelector('.submit-notice').classList.remove('.submit-failed');form.querySelector('.submit-success').classList.add('hidden');form.querySelector('.submit-failed').classList.add('hidden');}
